...
def filter_stopwords(text,additional_stopwords=[]):
    ...
    filtered_words = [word for word in word if word.lower() not in all_stopwords]
...
# function to genrate a download link for a data frame
def get_table_download_link(df, filename, file_label):
    csv = df.to_csv(index = False)
    b64 = base64.b64encode(csv.encoode()).decode()
    return f'<a href="data:file/csv;base64,{b64}" download="{filename}">{file_label}</a>'

# streamlit code
st.title("word Cloud Generator")
st.subheader("Upload a pdf, docx or title file to generate a word cloud")
...
if uploaded_file:
    ...
    elif uploaded_file.type=="application/vnd.openxmlformats-officedocument.wordprocessingml.documnet":
    ...
    use_standard_stopwords = st.sidebar.checkbox("Use standard stopwrods?", True)
    ...
    if text:
        ...
        width = st.sidebar.slider("Select Word Cloud Widht",400,2000,1200,50)
        height = st.sidebar.slider("Select Word Cloud Height", 200,2000,800,50)

        # genrate wordcloud
        st.subheader("Generator Word Cloud")
